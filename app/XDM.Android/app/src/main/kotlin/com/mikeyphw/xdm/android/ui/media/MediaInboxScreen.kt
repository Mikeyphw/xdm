...
fun MediaInboxScreen(
    ...
    val runningExternalJobs = termuxMediaPipeline.jobs.count {
        it.status == TermuxMediaJobStatus.Queued || it.status == TermuxMediaJobStatus.Running
    }
        ...
        XdmListCard(compact = true) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.Top) {
                Column(Modifier.weight(1f)) {
                    XdmCardTitle("Media")
                    XdmSupportingText(
                        "Review shared video, audio, HLS, DASH, or page links. Choose quality and tracks before adding anything to Downloads.",
                        maxLines = 3,
                    )
                }
                if (runningExternalJobs > 0) {
                    StatusPill("$runningExternalJobs processing", tone = XdmStatusTone.Info)
                }
            }
        }
            ...
            if (captures.isEmpty()) {
                item {
                    EmptyFeatureScreen(
                        ...
                        "Share a media or page link to XDM. It will appear here for review before any download starts.",
                    )
                }
            } else {
                items(captures, key = MediaCaptureRecord::id) { capture ->
                    ...
                    MediaCaptureCard(
                        ...
                        resolverPlanner = resolverPlanner,
                        onDownload = onDownload,
                        onResolve = onResolve,
                        onSelectVariant = onSelectVariant,
                        onTrackSelectionChanged = onTrackSelectionChanged,
                        onRemove = onRemove,
                        onTermuxMetadata = onTermuxMetadata,
                        onTermuxInspect = onTermuxInspect,
                        onTermuxYtDlpDownload = onTermuxYtDlpDownload,
                        onTermuxConvert = onTermuxConvert,
