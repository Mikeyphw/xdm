...
class RemediationPhase06_07ContractTest {
    ...
    @Test fun notificationsRecoveryAndFreshRestartUseExactDurableIdentity() {
        ...
        assertTrue(notifications.contains("terminalNotificationsLocked().firstOrNull { it.idempotencyKey == record.idempotencyKey }"))
        assertTrue(notifications.contains("\"terminal-dispatched\""))
    ...
    @Test fun destinationAndRecoverySurfacesUseLiveProviderAndReconciliationTruth() {
        ...
        assertTrue(add.contains("it.persistedWrite && it.status == DestinationHealthStatus.Healthy"))
        assertTrue(add.contains("headline = destination.displayName") && add.contains("supporting = destination.type.uiLabel()"))
