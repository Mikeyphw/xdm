...
class MediaNativeDirectDownloadPlanner {
    fun plan(
        ...
        val state = when {
            ...
            request.lane == MediaExecutionLane.NativeHlsSegmented || request.lane == MediaExecutionLane.YtDlpAdaptive || request.lane == MediaExecutionLane.EmbeddedFfmpegLive || request.lane == MediaExecutionLane.ProtectedBlocked -> NativeDirectRequestState.UnsupportedAdaptive
    ...
    private fun resumePlan(existingBytes: Long, request: MediaWorkerBridgeRequest): NativeDirectResumePlan {
        val support = when {
            request.lane == MediaExecutionLane.NativeHlsSegmented || request.lane == MediaExecutionLane.YtDlpAdaptive || request.lane == MediaExecutionLane.EmbeddedFfmpegLive || request.lane == MediaExecutionLane.ProtectedBlocked -> NativeDirectRangeSupport.NotSupported
