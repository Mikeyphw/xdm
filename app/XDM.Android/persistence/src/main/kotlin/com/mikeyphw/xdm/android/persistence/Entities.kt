...
data class AutomationCommandEntity(
    ...
    @ColumnInfo(defaultValue = "NULL") val claimedOriginPackage: String? = null,
    @ColumnInfo(defaultValue = "NULL") val verifiedIntegrationId: String? = null,
    @ColumnInfo(defaultValue = "'Untrusted'") val authorization: String = "Untrusted",
    @ColumnInfo(defaultValue = "0") val privateNetworkApproved: Boolean = false,
    @ColumnInfo(defaultValue = "0") val cleartextCredentialsApproved: Boolean = false,
...
@Entity(
    tableName = "post_processing_jobs",
    indices = [
        Index("rootJobId"),
        Index("subjectId"),
        Index("downloadId"),
...
data class PostProcessingJobEntity(
    @PrimaryKey val id: String,
    ...
    val outputDestinationUri: String?,
    val stagedOutputPath: String?,
    val outputBridgeUri: String?,
    val ownerBridgeUri: String?,
    val progressBridgeUri: String?,
    val metadataBridgeUri: String?,
    val payloadBridgeUri: String?,
    val finalOutputUri: String?,
    val publicationState: String,
    val publicationDisplayName: String?,
    val publicationExpectedBytes: Long?,
    val publicationExpectedSha256: String?,
    val committedOutputUri: String?,
    val committedBytes: Long?,
    val committedSha256: String?,
    val sideEffectOutcome: String?,
    val immutableSpecJson: String,
    val expectedSha256: String?,
    val actualSha256: String?,
    val requiredTools: String,
    val toolVersionsJson: String,
    val runId: String?,
    val executionId: Int?,
    val processToken: String?,
    val processId: Int?,
    val controlGeneration: Long,
    val requestedControl: String?,
    val progressPercent: Int,
    val progressBytes: Long,
    val progressTotalBytes: Long?,
    val timeoutAtEpochMs: Long?,
    val resultStdoutLength: Int,
    val resultStderrLength: Int,
...
@Entity(
    tableName = "post_processing_claims",
    foreignKeys = [
        ForeignKey(
            entity = PostProcessingJobEntity::class,
            parentColumns = ["id"],
            childColumns = ["jobId"],
    ...
    indices = [
        Index("jobId"),
        Index("subjectId"),
        Index(value = ["subjectId", "subjectGeneration", "trigger", "ruleId", "actionId"], unique = true),
    ],
)
data class PostProcessingClaimEntity(
    @PrimaryKey val claimKey: String,
    val subjectId: String,
    val subjectType: String,
    val subjectGeneration: Long,
    val trigger: String,
...
@Entity(
    tableName = "native_hls_jobs",
    foreignKeys = [
        ForeignKey(entity = MediaCaptureEntity::class, parentColumns = ["id"], childColumns = ["captureId"], onDelete = ForeignKey.CASCADE),
        ForeignKey(entity = DownloadEntity::class, parentColumns = ["id"], childColumns = ["downloadId"], onDelete = ForeignKey.CASCADE),
    ],
    indices = [
        Index("captureId"),
        Index("downloadId", unique = true),
        Index("stage"),
        Index(value = ["admissionKey", "attemptGeneration"], unique = true),
        Index("updatedAtEpochMs"),
    ],
)
data class NativeHlsJobEntity(
    @PrimaryKey val id: String,
    val captureId: String,
    val downloadId: String,
    val logicalMediaId: String,
    ...
    val unsupportedReasons: String,
    val stage: String,
    val finalizationState: String,
    val partCount: Int,
    val completedPartCount: Int,
    val bytesReceived: Long,
    val totalBytes: Long?,
    val progressPercent: Int,
    ...
    val fileName: String,
    val completedArtifactUri: String?,
    val completedArtifactBytes: Long?,
    val completedArtifactSha256: String?,
...
@Entity(
    tableName = "native_hls_parts",
    foreignKeys = [ForeignKey(entity = NativeHlsJobEntity::class, parentColumns = ["id"], childColumns = ["jobId"], onDelete = ForeignKey.CASCADE)],
    indices = [Index("jobId"), Index(value = ["jobId", "partIndex"], unique = true), Index("state"), Index("mediaSequence")],
)
data class NativeHlsPartEntity(
    @PrimaryKey val id: String,
    val jobId: String,
    val partIndex: Int,
    val url: String,
