...
object CompletedFileGrantPolicy {
    fun resolve(context: Context, download: Download, requireReadable: Boolean = true): Uri? {
        ...
        return when (parsed?.scheme?.lowercase()) {
            ContentResolver.SCHEME_CONTENT -> parsed.takeIf { !requireReadable || canRead(context, it) }
    ...
    private fun canRead(context: Context, uri: Uri): Boolean = runCatching {
        context.contentResolver.openFileDescriptor(uri, "r")?.use { descriptor -> descriptor.statSize >= -1L } == true
    }.getOrDefault(false)
}
