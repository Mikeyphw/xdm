...
$ADB -s "$serial" shell am start -a android.intent.action.VIEW -d 'xdmdownload://add?v=1&url=https%3A%2F%2Fexample.com%2Ffile.bin&filename=file.bin' "$pkg" | tee "$EVIDENCE_DIR/journey-external-add.txt"
$ADB -s "$serial" shell am start -a android.intent.action.VIEW -d 'xdmdownload://capture?v=3&url=https%3A%2F%2Fexample.com%2Fstream.m3u8&type=hls&title=Example' "$pkg" | tee "$EVIDENCE_DIR/journey-media-capture.txt"
python3 - "$EVIDENCE_DIR" "$serial" <<'PY'
from pathlib import Path
import json, sys, time
out=Path(sys.argv[1]); serial=sys.argv[2]
files=['journey-launch.txt','journey-external-add.txt','journey-media-capture.txt']
