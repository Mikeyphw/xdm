...
def validate(root: Path) -> list[str]:
    ...
    contains_all(app_db, ("PostProcessingJobEntity::class", "PostProcessingClaimEntity::class", "version = 22", "postProcessingDao"), "database", errors)
    ...
    for version in (17, 20):
        ...
        try:
            ...
            if version == 20:
                require("media_outputs" in names, "current schema lineage is missing media_outputs", errors)
...
def main() -> int:
    ...
    print("Durable automation, retained Room schema 17 plus current schema 22, exact process ownership, transient-session bridging, canonical root authorization, transactional publication, preflight, retry, recovery, UI, and regression contracts are present.")
    return 0
