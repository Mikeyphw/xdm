...
need(has(application, "NativeHlsMediaManager(", "transferRuntime.recoverForStartup()") and "nativeHlsRecovery" in application and "runCatching { nativeHlsMediaManager.recoverInterruptedJobs() }" in application, "application startup does not order native HLS recovery after publication-journal reconciliation")
