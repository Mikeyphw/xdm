...
require(manifest.get('current_overlay') in ({OVERLAY} | LATER_OVERLAYS) or current_overlay.startswith('XAR') or current_overlay.startswith('xdm_android_xar'), 'current overlay must point to Phase62 or a later accepted XAR overlay')
require(62 in manifest.get('project', {}).get('implemented_phases', []), 'implemented phases must include 62')
require(manifest.get('next_phase') in {'media_parity04_browser_ux_userscripts_feedback_final_release_seal', 'complete', 'phase63_release_readiness_support_bundle_seal', 'phase64_final_android_downloader_rc_seal', 'phase11_validation_matrix', 'XAR17_313_root_closure_audit_final_gate', 'xar17_313_root_closure_audit_final_gate', None}, 'next phase should mark completion, advance through the retained release seals, or point to XAR17 final closure')
require(phase.get('status') == 'implemented', 'Phase62 must be implemented')
require(phase.get('room_schema_unchanged') == 14 or manifest.get('database', {}).get('version', 0) >= 14, 'Phase62 must keep or be superseded by Room schema 14+')
