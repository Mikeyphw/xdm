...
require(manifest.get('current_overlay') == 'xdm_android_phase65_diagnostic_export_download_action_fix_overlay.zip', 'current overlay must be Phase65')
...
require(phase.get('room_schema_unchanged') == 14, 'Room schema must stay 14')
...
self_card = read('app/XDM.Android/app/src/main/kotlin/com/mikeyphw/xdm/android/ui/debug/RuntimeSelfTestSuiteCard.kt')
debug_screen = read('app/XDM.Android/app/src/main/kotlin/com/mikeyphw/xdm/android/ui/debug/DebugWorkbenchSettingsScreen.kt')
...
has(planner, 'deleteRecord(download, label = "Remove from list")', 'active/queued remove-from-list action missing')
...
has(vm, 'removeDownloadRecord(current.id)', 'delete record must route through cleanup helper')
has(vm, 'repository.deleteBackendTask(downloadId)', 'delete record must clear backend task ownership')
has(vm, 'repository.deleteRecoveryForDownload(downloadId)', 'delete record must clear recovery rows')
has(vm, 'repository.deleteFinalizationForDownload(downloadId)', 'delete record must clear finalization rows')
