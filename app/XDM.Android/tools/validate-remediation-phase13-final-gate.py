...
require("CurrentRoomSchema: Int = 25" in gate, "final media gate does not use Room schema 25")
...
require(manifest.get("current_overlay") in {"xdm_android_privacy_quality_final_gate_overlay_v2.zip", "xdm_android_ux01_ux09_product_foundation_v3.zip", "xdm_android_ux02_ux03_navigation_downloads_product_ui_v1.zip", "xdm_android_ux13_end_to_end_ui_ux_release_seal_v1.zip", "xdm_android_post_ux13_roadmap_completion_hotfix_v1.zip", "xdm_media_parity01_runtime_truth_diagnostics_backend_reliability_v2.zip", "xdm_media_parity02_logical_media_capture_browser_convergence_v2.zip", "xdm_media_parity03_native_hls_execution_admission_integrity_v2.zip", "xdm_media_parity04_browser_ux_userscripts_notifications_release_seal_v1.zip"} or current_overlay.startswith("XAR") or current_overlay.startswith("xdm_android_xar"), "PROJECT_MANIFEST current_overlay is not Overlay 13 or an accepted later XAR overlay")
database = manifest.get("database", {})
require(database.get("version") == 25, "PROJECT_MANIFEST authoritative database version is not 25")
require("18_to_19" in database.get("migrations", []) and "19_to_20" in database.get("migrations", []) and "20_to_21" in database.get("migrations", []) and "21_to_22" in database.get("migrations", []) and "22_to_23" in database.get("migrations", []) and "23_to_24" in database.get("migrations", []) and "24_to_25" in database.get("migrations", []), "PROJECT_MANIFEST migration chain does not reach schema 25")
public_gate = manifest.get("final_public_release_gate", {})
require(public_gate.get("room_schema_locked") == 25, "PROJECT_MANIFEST final public gate must report current Room schema 25")
