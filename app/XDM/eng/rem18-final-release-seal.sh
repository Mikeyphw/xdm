...
if [[ "$run_package" == 1 ]]; then
  if [[ "${RUNNER_OS:-$(uname -s)}" == Linux* || "$(uname -s 2>/dev/null || echo unknown)" == Linux* ]]; then
...
cat > "$artifacts_dir/final-seal-summary.json" <<JSON
{
  ...
  "packageMatrixAttempted": $([[ "$run_package" == 1 ]] && echo true || echo false),
