    ...
    private static MediaFormat? ParseFormat(JsonElement format)
    {
            ...
            && fragmentElements.ValueKind == JsonValueKind.Array)
        {
            ...
            foreach (JsonElement fragment in fragmentElements.EnumerateArray())
            {
                ...
                {
                    string uriKey = fragmentUri.AbsoluteUri;
                    occurrences.TryGetValue(uriKey, out int occurrence);
                    occurrences[uriKey] = occurrence + 1;
                    fragments.Add(new ExternalMediaFragment(ExternalFragmentDownloader.CreateStableId(fragmentUri, occurrence), fragmentUri));
                }
            }
        }
