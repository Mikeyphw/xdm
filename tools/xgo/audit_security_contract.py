...
def main()->int:
    ...
    report_dir=root/'.devtool/reports/xgo/security';report_dir.mkdir(parents=True,exist_ok=True)
    ...
    expected={'XGO-CAP-REQUEST-001':'XGO-20','XGO-CAP-SECURITY-001':'XGO-21','XGO-CAP-SECURITY-002':'XGO-21','XGO-CAP-SECURITY-003':'XGO-22','XGO-CAP-SECURITY-004':'XGO-23','XGO-CAP-SECURITY-005':'XGO-24','XGO-CAP-SECURITY-006':'XGO-25','XGO-CAP-PROXY-001':'XGO-25'}
    ...
    report={'schema_version':1,'status':'pass','security_capabilities':sorted(expected),'capability_count':len(ledger['capabilities']),'fixture_count':registry['fixture_count'],'shared_ledger_audit':json.loads(ledger_report.read_text()),'shared_fixture_audit':json.loads(fixture_report.read_text())}
